...
# HDMI TX CLOCK
set_property -dict { PACKAGE_PIN H16   IOSTANDARD TMDS_33 } [get_ports { hdmi_clk_p_0 }]; 
set_property -dict { PACKAGE_PIN H17   IOSTANDARD TMDS_33 } [get_ports { hdmi_clk_n_0 }];

# HDMI TX DATA (Bus de 3 bits)
# Canal 0 (Blue)
set_property -dict { PACKAGE_PIN D19   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_p_0[0] }]; 
set_property -dict { PACKAGE_PIN D20   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_n_0[0] }];

# Canal 1 (Green)
set_property -dict { PACKAGE_PIN C20   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_p_0[1] }]; 
set_property -dict { PACKAGE_PIN B20   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_n_0[1] }];

# Canal 2 (Red)
set_property -dict { PACKAGE_PIN B19   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_p_0[2] }]; 
set_property -dict { PACKAGE_PIN A20   IOSTANDARD TMDS_33 } [get_ports { hdmi_d_n_0[2] }];
...
## ----------------------------------------------------------------------------
## Pmod Header JD -> Usado para ADC (Pmod AD1)
## ---------------------------------------------------------------------------- 
# Pin 1 (Físico) -> CS    -> Pin T14 en FPGA                                                                                                                                                                                                                                                 
set_property -dict { PACKAGE_PIN T14   IOSTANDARD LVCMOS33     } [get_ports { spi_cs_0 }]; #IO_L5P_T0_34 Sch=jd_p[1] 
# Pin 2 (Físico) -> DATA0  -> Pin T15 en FPGA                 
set_property -dict { PACKAGE_PIN T15   IOSTANDARD LVCMOS33     } [get_ports { spi_sdata_0 }]; #IO_L5N_T0_34 Sch=jd_n[1]				 
#set_property -dict { PACKAGE_PIN P14   IOSTANDARD LVCMOS33     } [get_ports { jd[2] }]; #IO_L6P_T0_34 Sch=jd_p[2]  
# Pin 4 (Físico) -> SCLK  -> Pin R14 en FPGA                
set_property -dict { PACKAGE_PIN R14   IOSTANDARD LVCMOS33     } [get_ports { spi_sclk_0 }]; #IO_L6N_T0_VREF_34 Sch=jd_n[2]             
...
## ----------------------------------------------------------------------------
## Pmod Header JE -> Usado para DAC (Pmod DA1)
## ----------------------------------------------------------------------------
# Pin 1 (Físico) -> SYNC  -> Pin V12 en FPGA                                                                                                                 
set_property -dict { PACKAGE_PIN V12   IOSTANDARD LVCMOS33 } [get_ports { dac_nsync_0 }]; #IO_L4P_T0_34 Sch=je[1]						 
# Pin 2 (Físico) -> D0/D1 -> Pin W16 en FPGA
set_property -dict { PACKAGE_PIN W16   IOSTANDARD LVCMOS33 } [get_ports { dac_d1_0 }]; #IO_L18N_T2_34 Sch=je[2]                     
#set_property -dict { PACKAGE_PIN J15   IOSTANDARD LVCMOS33 } [get_ports { je[2] }]; #IO_25_35 Sch=je[3]                          
# Pin 4 (Físico) -> SCLK  -> Pin H15 en FPGA
set_property -dict { PACKAGE_PIN H15   IOSTANDARD LVCMOS33 } [get_ports { dac_clk_out_0 }]; #IO_L19P_T3_35 Sch=je[4]                     
...
# ----------------------------------------------------------------------------
# SUPRESIÓN AGRESIVA DE ERRORES HDMI SERDES
# ----------------------------------------------------------------------------
# Bajamos la severidad de todos los chequeos relacionados con relojes en OSERDES
set_property SEVERITY {Warning} [get_drc_checks REQP-1975]
set_property SEVERITY {Warning} [get_drc_checks REQP-1577]
set_property SEVERITY {Warning} [get_drc_checks REQP-1580]

# Opcional: Si lo anterior falla, intenta deshabilitarlos completamente:
# set_property IS_ENABLED 0 [get_drc_checks REQP-1975] 